...
import 'package:gyanbuddy/utils/const_dimensions.dart';
import 'package:gyanbuddy/models/itemdata_model.dart';
import 'package:gyanbuddy/utils/constants.dart'; // Adjust the import as per your project structure
...
class _PopupDialogState extends State<_PopupDialog> {
  ...
  late bool isSpeakingCompleted = false; //pronunciation complete
  ...
  void initState() {
    ...
    if (isAutoNextEnabled) {
      timer = Timer.periodic(const Duration(seconds: 4), (Timer t) {
        if (isSpeakingCompleted) { // Check if pronunciation is complete
          _nextItem();
          isSpeakingCompleted = false; // Reset the flag
        }
      });
    }
  }
  ...
  Future<void> _speakDescription() async {
    final currentItem = widget.items[currentIndex];
    await flutterTts.setLanguage("EN-IN");
    // Spell out the description letter by letter
    final description = currentItem.description;
    for (var i = 0; i < description.length; i++) {
      await flutterTts.speak(description[i]); // Speak each letter individually
      await Future.delayed(const Duration(milliseconds: 150)); // Add a slight delay between letters
    }
    await flutterTts.speak(currentItem.title); // Speak the number as a whole word
